...
import {
  ...
  createPrincipalSubject,
  updatePrincipalSubject,
  deletePrincipalSubject,
...
export default function Subjects() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updatePrincipalSubject(editingId, payload);
        showToast("Subject updated successfully");
      } else {
        await createPrincipalSubject(payload);
  ...
  const handleDeleteConfirm = async () => {
    setSaving(true);
    try {
      await deletePrincipalSubject(deleteTarget.id);
  ...
  return (
    <div className="w-full space-y-6 text-left animate-fadeIn">
      ...
      <DataTable
        ...
        filterSize="xl"
        exportable={true}
        exportFileName="principal-subjects-export"
      />
