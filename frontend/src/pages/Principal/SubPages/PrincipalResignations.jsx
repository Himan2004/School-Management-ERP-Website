...
// 🔥 Ensure you import from your Principal API service
import {
...
const PrincipalResignations = () => {
  ...
  useEffect(() => {
    const loadResignations = async () => {
      try {
        ...
        const response = await fetchPrincipalResignations();
        ...
        setError(err.message || "Failed to load staff resignations");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleApprove = async () => {
    ...
    try {
      setSubmitting(true);
      await approvePrincipalResignation(approveModalData._id, {
  ...
  const handleReject = async () => {
    ...
    try {
      setSubmitting(true);
      await rejectPrincipalResignation(rejectModalData._id, {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="w-full space-y-6 text-left pb-10"
      >
        <Heading
      ...
      <div>
        {loading ? (
          ...
          <DataTable
            title="School Resignation Records"
            size={12}
            searchable={true}
            exportable={true}
            exportFileName="staff_resignations"
            pageSize={5}
            columns={[
              {
                key: "staff",
              ...
              },
              {
                ...
                searchValue: (row) => row?.staffId?.role || "",
              },
              // 🔥 Branch column removed since it's all one school
              {
                key: "appliedOn",
                label: "Applied On",
                render: (_, row) => formatDate(row.resignationDate),
                sortValue: (row) => new Date(row.resignationDate).getTime()
              },
              {
                key: "lwd",
                label: "Last Working Day",
                render: (_, row) => formatDate(row.lastWorkingDate),
                sortValue: (row) => new Date(row.lastWorkingDate).getTime()
              },
              {
                key: "status",
                label: "Status",
        ...
        {selectedResignation && (
          <div className="flex flex-col gap-6">
            ...
            <ModalGrid cols={2} title="Employee Details">
              ...
              <ModalData label="Status" value={toTitle(selectedResignation.status)} />
...
export default PrincipalResignations;