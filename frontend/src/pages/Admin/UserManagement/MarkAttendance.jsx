...
const MarkAttendance = () => {
  ...
  const loadAttendance = useCallback(async () => {
    try {
      ...
      const staff = (staffRes.data?.data || []).filter((item) =>
        ["teacher", "accountant"].includes(item.role)
      );

      const reportRes = await api.get("/admin/staff/attendance/report", {
        params: {
      ...
      const attendanceMap = {};
      (reportRes.data?.data || []).forEach((record) => {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      await Promise.all(
        staffList.map((staff) =>
          api.post("/admin/staff/attendance", {
            staffId: staff._id,
            date: selectedDate,
  ...
  const handleApproveLeave = async (id) => {
    try {
      setError("");
      const response = await api.patch(`/admin/staff/leave/${id}/process`, { status: "approved" });
      setToastMessage(response.data?.message || "Leave request approved.");
      setTimeout(() => setToastMessage(""), 3500);
      await loadLeaves();
    } catch (err) {
  ...
  const handleRejectLeaveConfirm = async () => {
    ...
    try {
      ...
      setError("");
      const response = await api.patch(`/admin/staff/leave/${selectedLeaveId}/process`, {
        status: "rejected",
        rejectionReason,
      });
      setToastMessage(response.data?.message || "Leave request rejected.");
      setTimeout(() => setToastMessage(""), 3500);
