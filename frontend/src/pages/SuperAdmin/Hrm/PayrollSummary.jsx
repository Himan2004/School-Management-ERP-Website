...
const PayrollSummary = () => {
      ...
      try {
        ...
        const query =
          selectedMonth && selectedYear
            ...
            : "";
        const payrollResponse = await fetchPayroll(query);
        ...
        // Dynamically populate the school dropdown from the backend filters object
        if (payrollResponse?.filters?.schools) {
          setAvailableSchools(payrollResponse.filters.schools);
        }
      } catch (err) {
  ...
  // Handle frontend filtering for the selected school
  const filteredRows = useMemo(() => {
    if (selectedSchool === "All Schools") return payrollRows;
    return payrollRows.filter((row) => {
      const schoolName = row?.school?.schoolName || row?.school?.name;
      return schoolName === selectedSchool;
    });
  }, [payrollRows, selectedSchool]);

  const totals = useMemo(() => {
    const totalSalary = filteredRows.reduce((sum, row) => sum + (row.netSalary || 0), 0);
    const draftCount = filteredRows.filter((row) => row.payrollStatus === "draft").length;
    const approvedCount = filteredRows.filter((row) => row.payrollStatus === "approved").length;
  ...
  const patchStatus = (id, status) => {
    setPayrollRows((prev) =>
      prev.map((row) =>
        row._id === id ? { ...row, payrollStatus: status } : row,
      ),
    );
    setSelectedPayroll((prev) =>
      prev && prev._id === id ? { ...prev, payrollStatus: status } : prev,
    );
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      setSubmitting(true);
      await approvePayroll(approvalPayroll._id, { remarks });
      patchStatus(approvalPayroll._id, "approved");
      toast.success("Payroll approved");
      closeModal("approve-modal");
      setTimeout(() => {
        setApprovalPayroll(null);
        setRemarks("");
      }, 300);
    } catch (err) {
  ...
  const handleReject = async () => {
    ...
    try {
      setSubmitting(true);
      await rejectPayroll(rejectPayrollRow._id, {
  ...
  const openApproveModal = (row) => {
    setApprovalPayroll(row);
    setRemarks("");
    setTimeout(() => openModal("approve-modal"), 50);
  };

  const openRejectModal = (row) => {
    setRejectPayrollRow(row);
    setRejectReason("");
  ...
  return (
    ...
    >
      <Heading
        ...
        icon={<Banknote className="text-[#223F74] w-6 h-6 sm:w-8 sm:h-8" />}
        size={12}
        showAnimations
      />
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        <div className="w-full sm:w-48">
          ...
          >
            <Option value="All Schools" label="All Schools" />
      ...
      <Grid cols={12} gap={4}>
        {loading ? (
          <div className="col-span-12 py-20 flex flex-col items-center justify-center gap-3 text-slate-500 bg-white rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <span className="font-semibold text-sm">Loading payroll records...</span>
          </div>
        ) : (
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: "branch",
                label: "Branch",
                render: (_, row) => row?.school?.schoolName || row?.school?.name || "-",
              },
              {
                key: "month",
                label: "Month",
                render: (_, row) => formatDate(row.payrollMonth),
              },
              {
                key: "salary",
                label: "Salary Amount",
                align: "right",
                render: (_, row) => formatCurrency(row.netSalary || 0),
              },
              {
                key: "status",
                label: "Status",
            ...
            actions={[
              {
                ...
                variant: "ghost",
                onClick: (row) => setSelectedPayroll(row),
              },
              {
                icon: <CheckCircle2 size={13} />,
                tooltip: "Approve",
                variant: "primary",
                show: (row) => row.payrollStatus === "draft",
      ...
      <PanelModal
        id="view-payroll-modal"
        title="Payroll Details"
        isVisible={!!selectedPayroll}
          ...
          <div className="flex flex-col gap-6">
            <div>
              <h3 className="text-2xl sm:text-3xl font-black text-slate-800">
                {selectedPayroll?.staffId?.name || "-"}
              </h3>
              <p className="text-slate-500 font-bold mt-1 text-sm sm:text-base">
                {selectedPayroll?.school?.schoolName || selectedPayroll?.school?.name || "-"}
              </p>
            </div>
            ...
            <ModalGrid cols={2} title="Salary Information">
              ...
              <ModalData label="Status" value={
                <span className="uppercase tracking-widest text-xs font-black">{selectedPayroll.payrollStatus}</span>
              } />
            </ModalGrid>
          ...
          <Grid cols={12} gap={4}>
            <div className="col-span-12 mb-2 p-4 rounded-xl bg-blue-50/50 border border-blue-100 text-sm">
              <p className="font-bold text-slate-700">
                ...
                <span className="font-semibold text-blue-700">
                  {approvalPayroll?.staffId?.name}
                </span>
              </p>
              <p className="font-bold text-slate-700 mt-1">
                Net Pay:{" "}
                <span className="font-black text-emerald-600">
                  {formatCurrency(approvalPayroll?.netSalary)}
                </span>
              </p>
            </div>
            <DataField
            ...
            <div className="col-span-12 flex gap-3 mt-4">
              <Button
                ...
                onClick={() => {
                  closeModal("approve-modal");
                  setTimeout(() => setApprovalPayroll(null), 300);
                }}
                size={6}
              />
              <Button
                text={submitting ? "Processing..." : "Confirm Approval"}
                variant="primary"
                disabled={submitting}
                loading={submitting}
          ...
          <Grid cols={12} gap={4}>
            ...
            <div className="col-span-12 flex gap-3 mt-4">
              <Button
                ...
                onClick={() => {
                  closeModal("reject-modal");
                  setTimeout(() => setRejectPayrollRow(null), 300);
                }}
                size={6}
              />
              <Button
