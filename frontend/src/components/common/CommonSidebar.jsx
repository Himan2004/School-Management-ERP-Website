...
// ── Main Sidebar ──────────────────────────────────────────────────────────────
const CommonSidebar = memo(function CommonSidebar({ 
  expanded, 
  onExpand, 
  onNavClick, 
  onCollapse,
  ...
  const isActive = (item) => {
    if (item.matches) {
      return item.matches.some((pattern) =>
        matchPath(
          { path: pattern, end: pattern === item.path || item.end === true },
          location.pathname
        )
      );
    }
  ...
  const isSectionActive = (children) => children?.some((c) => isActive(c));
    ...
    >
      ...
      <div className={`mt-auto ${expanded ? 'px-3 pt-3 pb-4' : 'px-2 pt-3 pb-4 flex justify-center'}`}>
        {expanded ? (
          ...
          >
            <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br ${badgeColors} text-xs font-bold text-white shadow-sm`}>
              {roleInitials}
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className="text-[13px] font-semibold truncate" style={{ color: C.text }}>{roleName}</p>
          ...
          <Tooltip label="Logout">
            <button
              type="button"
              onClick={() => setShowLogoutModal(true)}
              className={`flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br ${badgeColors} text-xs font-bold text-white shadow-sm cursor-pointer hover:from-rose-400 hover:to-rose-600 transition-all duration-200`}
            >
              {roleInitials}
            </button>
          </Tooltip>
