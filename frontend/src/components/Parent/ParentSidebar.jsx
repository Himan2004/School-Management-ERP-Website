...
import {
  ...
  LogOut, Menu, Home, School, Award, Receipt, Bell, Heart, IdCard,
  Sparkles, Users, MessageSquare, X, Settings, ChevronDown
} from 'lucide-react';
import { FaUserNurse, FaUserShield } from 'react-icons/fa';
import { MdDescription } from 'react-icons/md';
import { useDispatch } from 'react-redux';
import { parentLogout } from '../../features/auth/parentAuthSlice.js';
...
// ── Main Sidebar ──────────────────────────────────────────────────────────────
const ParentSidebar = memo(function ParentSidebar({ expanded, onExpand, onNavClick, onCollapse }) {
  ...
  const handleLogoutConfirm = useCallback(async () => {
    ...
    try {
      await dispatch(parentLogout()).unwrap();
  ...
  const menuItems = [
    ...
    // { label: "Community", path: "/parent/community", icon: Users },
    { label: "Meetings", path: "/parent/meetings", icon: Calendar },
    { label: "ID Card", path: "/parent/id-card", icon: IdCard },
    { label: "Health Records", path: "/parent/health", icon: Heart },
    { label: "Student Profile", path: "/parent/profile", icon: User },
    { label: "Ticket System", path: "/parent/ticket-system", icon: MessageSquare },
    ...
    >
      ...
      <div className={`mt-auto ${expanded ? 'px-3 pt-3 pb-4' : 'px-2 pt-3 pb-4 flex justify-center'}`}>
        {expanded ? (
          ...
          >
            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-fuchsia-400 to-purple-600 text-xs font-bold text-white shadow-sm">
              PT
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className="text-[13px] font-semibold truncate" style={{ color: C.text }}>Parent</p>
          ...
          <Tooltip label="Logout">
            <button
              type="button"
              onClick={() => setShowLogoutModal(true)}
              className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-400 to-purple-600 text-xs font-bold text-white shadow-sm cursor-pointer hover:from-rose-400 hover:to-rose-600 transition-all duration-200"
            >
              PT
            </button>
          </Tooltip>
