...
import {
  Home, BookOpen, Calendar, Award, Clock, Bus, Ticket, Eye,
  MessageSquare, Settings, LogOut, GraduationCap, ChevronLeft,
  ChevronRight, X, TrendingUp, Activity, Target, Trophy, Star,
  BarChart, PieChart, Zap, Heart, FileText, Bell, Video,
  Download, Upload, Users, HeartPulse, IdCard, AlertCircle,
  Megaphone, Menu, ChevronDown, Scale
} from 'lucide-react';
import { useDispatch } from 'react-redux';
import { studentLogout } from '../../features/auth/studentAuthSlice.js';
...
// ── Main Sidebar ──────────────────────────────────────────────────────────────
const StudentSidebar = memo(function StudentSidebar({ expanded, onExpand, onNavClick, onCollapse }) {
  ...
  const menuItems = [
    ...
    { icon: Scale, label: 'Legal & Policies', path: '/student/terms-privacy' },
    ...
    >
      ...
      <div className={`mt-auto ${expanded ? 'px-3 pt-3 pb-4' : 'px-2 pt-3 pb-4 flex justify-center'}`}>
        {expanded ? (
          ...
          >
            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-400 to-purple-600 text-xs font-bold text-white shadow-sm">
              ST
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className="text-[13px] font-semibold truncate" style={{ color: C.text }}>Student</p>
          ...
          <Tooltip label="Logout">
            <button
              type="button"
              onClick={() => setShowLogoutModal(true)}
              className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-400 to-purple-600 text-xs font-bold text-white shadow-sm cursor-pointer hover:from-rose-400 hover:to-rose-600 transition-all duration-200"
            >
              ST
            </button>
          </Tooltip>
